...
export const Story = ({
  ...
  const effectiveSwipeThreshold = swipeThreshold ?? SWIPE_THRESHOLD;
  const handleSwipeBeyondEnd = useCallback(() => {
    setIsModalOpen(false);
    setCurrentPage(0);
    if (onClose && selectedData.length) {
      onClose(selectedData[selectedData.length - 1]);
    }
  }, [onClose, selectedData, currentPage]);
    ...
    if (Platform.OS == 'ios') {
      return (
        <CubeNavigationHorizontal
          ...
          swipeThreshold={effectiveSwipeThreshold}
          onSwipeBeyondEnd={handleSwipeBeyondEnd}
